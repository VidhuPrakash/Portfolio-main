...
import { Briefcase, GraduationCap, HomeIcon, Mail, User } from "lucide-react";
      ...
      <Dock className="items-end pb-3 rounded-full">
        {data.map((item, idx) => (
          <Link href={item.href} key={idx}>
            <DockItem
              className={cn(
                "aspect-square rounded-full bg-black dark:bg-neutral-800",
                pathname === item.href && " bg-gray-800 border border-primary}"
              )}
            >
              <DockLabel>{item.title}</DockLabel>
              <DockIcon
                className={cn(pathname === item.href && "text-[#ffffff]")}
              >
                {item.icon}
              </DockIcon>
            </DockItem>
